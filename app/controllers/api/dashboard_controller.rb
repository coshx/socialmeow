class Api::DashboardController < ApiController
  ...
  def index
    ...
    users.each do |u|
	    ...
	    result[:days_of_word] = (last_date - first_date).to_i / 1.day
	    if result[:days_of_word] > 0
		    result[:followed_per_day] = result[:accounts_followed] / result[:days_of_word]
		    result[:followed_back_per_day] = result[:accounts_followed_back] / result[:days_of_word]
		end
	    results << result
   	end
